...
module.exports = function user(options) {
  ...
  seneca.add({role:role, cmd:'getRegisteredUsers'},function(args,done){
  ...
  //Get registered users
  seneca.add({role:role, cmd:'getUserByEmail'},function(args,done){
    var userent  = seneca.make('sys/user')
    userent.list$({email:args.email},function(err,user){
      done(null, user )
    })
  })
  ...
  function resolve_user( cmd, fail ) {
    return function( args, done ) {
      ...
      if( args.user && args.user.entity$ ) {
        ...
        if( !valid ) {
          return done(null,{ok:false,why:'Name or email missing'})
        }
  ...
  function cmd_encrypt_password( args, done ){
    ...
    if( _.isUndefined( password ) ) {
      ...
      else return seneca.fail({code:'No password provided',whence:args.whence},done);
    }

    if( _.isUndefined( repeat ) ) {
      if( options.mustrepeat ) {
        return seneca.fail({code:'Password cannot be repeated',whence:args.whence},done);
      }
      else repeat = password
    }

    if( password !== repeat ) {
      return done(null,{ok:false,why:'Password mismatch',whence:args.whence})
    }

    if( password.length<2) {
      return done(null,{ok:false,why:'Password too short',whence:args.whence})
    }
  ...
  function cmd_register(args,done){
    ...
    user.date     = new Date().toISOString().substring(0, 19).replace('T', ' ')
  ...
  function cmd_execute_reset( args, done ){
    ...
    resetent.load$(q, function( err, reset ) {
      ...
      if( new Date() < new Date(reset.date)+ options.resetperiod ) {
